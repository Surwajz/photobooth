...
const frameOptions = [
    ...
    "/assets/frames/heart-frame-5.png",
];

const stickerOptions = [
    "/assets/stickers/leaf.png",
    "/assets/stickers/sparkles.png",
    "/assets/stickers/puppy.png",
    "/assets/stickers/star.png",
];

const videoConstraints = { width: 953, height: 599, facingMode: "user" };

// Default photo-slot layout (in frame-image pixels), designed for the
// 1200x3000 frames. Frames listed in FRAME_SLOT_CONFIGS below override this.
...
// Frames 4 & 5 are smaller (793px wide) with differently positioned and
// sized photo windows, so each gets its own slot rectangles. Any frame not
// listed here keeps the default 1200x3000 layout above (untouched).
const FRAME_SLOT_CONFIGS = {
    "/assets/frames/heart-frame-4.png": [
        { x: 117, y: 225, width: 559, height: 351 },
        { x: 118, y: 615, width: 557, height: 383 },
        { x: 118, y: 1036, width: 557, height: 366 },
        { x: 119, y: 1441, width: 558, height: 382 }
    ],
    "/assets/frames/heart-frame-5.png": [
        { x: 99, y: 86, width: 576, height: 354 },
        { x: 101, y: 504, width: 578, height: 365 },
        { x: 101, y: 923, width: 579, height: 349 },
        { x: 102, y: 1335, width: 578, height: 301 }
    ]
};
...
export default function PhotoBooth() {
    ...
    // Slot geometry depends on the selected frame: frames 4 & 5 use their own
    // window rectangles, all other frames keep the default 1200x3000 layout.
    const slotRects = FRAME_SLOT_CONFIGS[selectedFrame]
        ? FRAME_SLOT_CONFIGS[selectedFrame]
        : DEFAULT_SLOTS.map(s => ({ x: s.x, y: s.y, width: SLOT_WIDTH, height: SLOT_HEIGHT }));
    const slots = slotRects.map(s => ({ x: s.x, y: s.y }));
    ...
    const drawCanvas = () => {
        ...
        photos.forEach(p => {
            const slot = slotRects[p.slotIndex];
    ...
    const addPhoto = img => {
        ...
        // Fit the photo into the destination slot. Frames 4 & 5 (which have
        // their own slot rects) use a "cover + center" fit so each photo
        // fills its whole window; every other frame keeps the original
        // width-fill behaviour exactly.
        const slot = slotRects[photoCount];
        const usesDynamicSlots = FRAME_SLOT_CONFIGS[selectedFrame] != null;

        const scale = usesDynamicSlots
            ? Math.max(slot.width / img.width, slot.height / img.height)
            : slot.width / img.width;

        const drawW = img.width * scale;
        const drawH = img.height * scale;

        const offsetX = usesDynamicSlots ? (slot.width - drawW) / 2 : 0;
        const offsetY = usesDynamicSlots
            ? (slot.height - drawH) / 2
            : (drawH > slot.height ? (slot.height - drawH) / 2 : 0);
